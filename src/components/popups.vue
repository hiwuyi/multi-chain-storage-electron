<template>
  <div :class="{'slideScroll slideAdd': true, 'slideAddBg': dialogFormVisible&&(typeName !== 'add'), 'slideComSoon': fixed, slideEmail: dialogFormVisible&&(typeName === 'emailLogin'||typeName === 'emailCheck'||typeName === 'upload_folder_list')}"
    ...
    <div class="fe-none" v-if="typeName === 'login_apikey'">
      <div class="addBucket" v-loading="createLoad">
        <i class="el-icon-circle-close close" @click="closeDia()"></i>
        <div class="title">Apikey login</div>
        <el-form :model="form" status-icon :rules="rulesLogin" label-position="top" ref="form" @submit.native.prevent>
          <el-form-item prop="login_apikey" :label="$t('my_profile.create_api_tips03')">
            <el-input v-model="form.login_apikey"></el-input>
          </el-form-item>
          <el-form-item prop="login_token" :label="$t('my_profile.create_api_tips04')">
            <el-input v-model="form.login_token"></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <div class="login_tip">
          API Key and Access Token - Generated via
          <a href="https://multichain.storage" target="_blank">https://multichain.storage</a>.
        </div>
      </div>
    </div>
        ...
        <div class="titleCont">
          <div class="address">
            <div class="address_left">
              Apikey: {{ apikeyAccount | hiddAddress}}
            </div>
...
export default {
  data () {
    ...
    return {
      ...
      rulesLogin: {
        login_apikey: [{ required: true, message: ' ', trigger: 'blur' }],
        login_token: [{ required: true, message: ' ', trigger: 'blur' }]
      },
      rulesEmail: {
        email: [
  ...
  methods: {
    ...
    async getUserDescribe (name) {
      const describeForNameMap = {
        'add_apikey': () => { return that.form.day },
        'add_domain': () => { return that.form.domain },
        'login_apikey': () => { return that.form }
      }
      return describeForNameMap[name] ? describeForNameMap[name]() : that.form.name
    },
    getDialogClose (formName, type) {
      ...
      that.$refs[formName].validate(async valid => {
        if (valid) {
          const resCont = await that.getUserDescribe(that.typeName)
          that.$emit('getPopUps', false, that.typeName, resCont)
        } else {
  ...
  computed: {
    ...
    apikeyAccount () {
      return this.$store.getters.apikeyAccount
    }
  },
  filters: {

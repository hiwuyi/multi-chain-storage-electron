<template>
  <div class="metamaskHome">
    ...
    <pop-ups v-if="loginDialog" :dialogFormVisible="loginDialog" :createLoad="createLoad" :typeModule="typeName" :fixed="true" @getPopUps="getPopUps"></pop-ups>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      loginDialog: false,
      createLoad: false,
      typeName: 'login_apikey'
    }
  },
  components: {
    vHead, vFoot, CarouselContainer, networkAlert, vStats, popUps
  },
  methods: {
    async getPopUps (dialog, bucketName, rows) {
      if (!rows) {
        that.loginDialog = dialog
        return
      }
      that.createLoad = true
      const params = {
        apikey: rows.login_apikey,
        access_token: rows.login_token,
        network: 'polygon.mainnet'
      }
      const respon = await that.$commonFun.sendRequest(`${process.env.BASE_PAYMENT_GATEWAY_API}api/v1/user/login_by_api_key`, 'post', params)
      if (!respon || respon.status !== 'success') {
        that.$message.error(respon.message || 'Fail')
        that.createLoad = false
        return
      } else {
        that.$store.dispatch('setMCSjwtToken', respon.data.jwt_token)
        that.$store.dispatch('setApikeyAccount', rows.login_apikey)
      }
      const lStatus = await that.$metaLogin.login(respon.data.jwt_token)
      if (lStatus) setTimeout(function () { window.location.reload() }, 200)
      else that.createLoad = false
    },
    goLink (link) {
      window.open(link)
    },
